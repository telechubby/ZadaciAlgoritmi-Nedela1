...
public class Zadaca2Resenie {
    public static void balanceBoxers(DLL<Boxer> lightWeight, DLL<Boxer> heavyWeight){
        //TODO: your code here
        DLLNode<Boxer> najslab=heavyWeight.getFirst();
        DLLNode<Boxer> current=heavyWeight.getFirst();
        while(current!=null){
            if(current.element.calculateStrength()<najslab.element.calculateStrength())
                najslab=current;
            current=current.succ;
        }
        lightWeight.insertLast(najslab.element);
        heavyWeight.delete(najslab);
        DLLNode<Boxer> najjak=lightWeight.getFirst();
        current=lightWeight.getFirst();
        while(current!=null){
            if(current.element.calculateStrength()>najjak.element.calculateStrength())
                najjak=current;
            current=current.succ;
        }
        heavyWeight.insertFirst(najjak.element);
        lightWeight.delete(najjak);
    }
